...
void ql_l1_finished(tap_dance_state_t *state, void *user_data) {
    ...
    switch (ql_tap_state.state) {
        ...
        case TD_SINGLE_HOLD: case TD_DOUBLE_TAP: layer_on(1); break;
        default:
            break;
    }
}
...
void ql_md_ult_finished(tap_dance_state_t *state, void *user_data) {
    ...
    switch (ql_tap_state.state) {
        ...
        case TD_DOUBLE_HOLD: register_code(KC_MNXT); break;
        case TD_TRIPLE_TAP:  tap_code(KC_MPRV); break;
        case TD_TRIPLE_HOLD: register_code(KC_MPRV); break;
        default: break;
    }
}
...
void ql_md_ult_reset(tap_dance_state_t *state, void *user_data) {
    // If the key was held down and now is released then switch off the layer
    switch (ql_tap_state.state) {
        case TD_DOUBLE_HOLD: unregister_code(KC_MNXT); break;
        case TD_TRIPLE_HOLD: unregister_code(KC_MPRV); break;
        default: break;
    }
    ql_tap_state.state = TD_NONE;
